...
import type { ProjectData, Scene } from '../../src/types';

/**
 * 确保项目目录结构完整
 */
function ensureProjectStructure(projectDir: string) {
 ...
 * 保存项目
 */
ipcMain.handle('project:save', async (_event, data: ProjectData, projectPath: string) => {
  try {
    ...
    // 写入新场景文件
    for (const scene of Object.values(data.scenes) as Scene[]) {
...
ipcMain.handle('project:load', async (_event, projectPath: string) => {
  try {
    ...
    throw new Error(`加载项目失败: ${(e as Error).message}`);
  }
});
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      ...
      throw new Error(`导出失败: ${(e as Error).message}`);
    }
  }
);
